      ...
      <main className="max-w-6xl mx-auto px-4 py-8 md:py-12">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          <div className="mb-12">
            ...
            <p className="text-slate-400">All your saved countries, words, and conversions</p>
          </div>

          {loading ? (
            <div className="text-center py-12">
              <div className="inline-block animate-spin rounded-full h-12 w-12 border-4 border-slate-600 border-t-blue-600" />
            ...
            <Card hover={false}>
              <div className="text-center py-12">
                ...
                <p className="text-xl text-slate-400 mb-6">No favorites yet!</p>
                <p className="text-slate-500 mb-8">Start saving your favorite countries, words, and conversions to see them here.</p>
                <a href="/explore" className="text-blue-400 font-semibold hover:underline">Go explore \u2192</a>
              </div>
            </Card>
          ) : (
            <div className="space-y-12">
              {favoriteCountries.length > 0 && (
                <div>
                  <h2 className="text-2xl font-bold text-white mb-6">\uD83C\uDF0D Favorite Countries ({favoriteCountries.length})</h2>
                        ...
                        <Card>
                          <div className="flex gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                              <h3 className="font-semibold text-white mb-1 truncate">{c.name.official}</h3>
                              <p className="text-sm text-slate-400 mb-4">Pop: {(c.population / 1_000_000).toFixed(1)}M{c.capital && ` \u2022 ${c.capital[0]}`}</p>
              ...
              {wordFavorites.length > 0 && (
                <div>
                  <h2 className="text-2xl font-bold text-white mb-6">\uD83D\uDCDA Favorite Words ({wordFavorites.length})</h2>
              ...
              {conversionFavorites.length > 0 && (
                <div>
                  <h2 className="text-2xl font-bold text-white mb-6">\uD83D\uDCB1 Favorite Conversions ({conversionFavorites.length})</h2>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {conversionFavorites.map((conv) => (
                      <motion.div key={conv} layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }}>
                        <Card className="flex items-center justify-between">
                          <span className="font-semibold text-white">{conv.replace('-', ' \u2192 ')}</span>
